...
<body>
	...
	<div class="page-header table-responsive">
		...
		<table class="grid table table-striped table-hover table-bordered">
			...
			<tr>
				...
				<td class="ck-content"><div style="min-height: 400px;"><p>${board.cbBoContent}</div></p></td>
			</tr>
			<tr>
				<th>첨부파일</th>
					...
					<div class="pull-left">
						<form action="clubMainView.lol" name="myform" method="post">
							<input type="hidden" value="${board.cbBoClubNum}" name="bkNum">
								<button type="submit" class="btn btn-primary btn-xs blue" ><i class="fa fa-list"></i>목록</button>
						</form>
					</div> <c:if test="${sessionScope.USER_INFO.userID eq board.cbBoMemId && not empty sessionScope.USER_INFO.userID || sessionScope.USER_INFO.userRole eq '관리자'}">
	...
	<form name="formReply" action="<c:url value='/reply/replyRegist.lol'/>" method="post">
		...
		<input type="hidden" name="reBoardNum" value="${board.cbBoNum}">
		<input type="hidden" name="reCategory" value="CLUB${board.cbBoNum}${board.cbBoClubNum}">
...
<!-- 					<div class="jumbotron"> -->
<%-- 					<input type="hidden" name="reBoardNum" value="${board.cbBoNum}"> --%>
<%-- 					<input type="hidden" name="reCategory" value="CLUB${board.cbBoNum}${board.cbBoClubNum}"> <input --%>
<!-- 						type="hidden" name="reMemId" -->
<%-- 						value="${sessionScope.USER_INFO.userID}"> <input --%>
<!-- 						type="hidden" name="reWriter" -->
...
$(document).ready(function() {
	...
	$('#btn_reply_regist').on("click", function() {
		...
		if(content == ''){
			...
			params = {
				"reBoardNum" : "${board.cbBoNum}",
				"reCategory" : "CLUB${board.cbBoNum}${board.cbBoClubNum}",
...
// 					str += '<div class="jumbotron">';
					...
					str += '<div class= "row" style="margin-left: ' + row.reDepth*50 + 'px;">';
          ...
          str += '  <input type="hidden" name="reContent" value="'+row.reContent+'">    ';
					...
					str += '	<input type="hidden" name="reBoardNum" value="${board.cbBoNum}">                  '
					str += '	<input type="hidden" name="reCategory" value="CLUB${board.cbBoNum}${board.cbBoClubNum}">             '
					...
					str += '				class="btn btn-primary btn-xs green " onclick=registReplyDeeper(this)>등록</button>'
					str += '		</div>                                                                        '
					str += '	</div>                                                                            '
					str += '</form>                                                                               '
